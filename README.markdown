...
4. Once a Bluetooth or USB connection is established, you can enqueue subclasses of `MRNXTCommand` onto the device's work queue. If the specified response callback is not `NULL`, an instance of `MRNXTResponse` will be passed to the callback block. Commands execute serially in FIFO order.
